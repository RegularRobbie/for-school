...
if choice == "1":
    ...
    message = " Roberts first class is  " +  classFirst  + " Then he goes to  " + classSecond + "Then he goes home"
    ...
    fileOfFiles = input("Please input name of the file:  ")
    fList = []
    for p in pathlib.Path('.').iterdir():
        if p.is_file():
            fList.append(p)
    with open(fileOfFiles, "w") as myFile:
        myFile.write("my list of files: \n ")
        for f in fList:
            myFile.write(f.name)
            myFile.write("\n")
